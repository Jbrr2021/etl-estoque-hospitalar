...
Este projeto simula um pipeline de Engenharia de Dados para controle de estoque hospitalar.

A ideia do projeto é praticar conceitos fundamentais de Engenharia de Dados, como extração de dados, transformação, validação, carga em banco de dados e consultas SQL para análise.
...
6. Carrega os dados tratados em um banco SQLite.
7. Executa consultas SQL para análise dos dados.
...
Arquivo criado para testar se o Pandas estava instalado e funcionando corretamente.
...
Caso alguma validação falhe, o pipeline exibe um erro.
...
Validação concluída com sucesso.
Dados carregados com sucesso em: C:\Users\User\Documents\etl-estoque-hospitalar\database\estoque.db
Pipeline finalizado com sucesso.
```

Após essa execução, o arquivo `estoque.db` será criado dentro da pasta `database`.
...
de estoque atual. Depois, os dados são validados e carregados em um banco SQLite. 
Também criei consultas SQL para analisar materiais abaixo do estoque mínimo, saídas 
por setor e movimentações por categoria.
```
...
3. Migrar o banco de SQLite para PostgreSQL.
4. Criar dashboard no Power BI ou Looker Studio.
5. Criar versão em cloud usando Google Cloud.
6. Usar BigQuery como destino dos dados.
7. Automatizar a execução do pipeline.
8. Criar logs de execução.
...
Estudante de Engenharia da Computação, com interesse em Engenharia de Dados, Cloud, Automação, IA, SQL, Python e soluções voltadas para organização e melhoria de processos.